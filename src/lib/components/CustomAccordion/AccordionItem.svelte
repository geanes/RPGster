<script>
	...
	function setActive(event) {
		if (autocollapse === true) {
			$active === id ? active.set() : active.set(id);
		} else {
			open = !open;
		}
		onToggle(event);
...
<div class="accordion-item {classesBase} hover:text-slate-100" data-testid="accordion-item">
	<!-- Control -->
	<div
		class="flex border-t-[1px] py-1 pl-2 border-slate-500/30 shadow-md"
		class:bg-surface-700={openState}
	>
		<!-- Lead -->
		{#if $$slots.lead}
			<div class="accordion-lead mr-2 flex place-items-center">
				<slot name="lead" />
			</div>
		{/if}
		<!-- Summary -->
		...
		<div class="mr-2">
			<button
				type="button"
				class="accordion-control {classesControl} py-4 justify-self-end"
				id="accordion-control-{id}"
				on:click={setActive}
				on:click
				on:keydown
